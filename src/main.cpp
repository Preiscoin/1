...
uint256 hashGenesisBlock("0x2ffbf36d8fe734317177ece5e985a7fc651735ff6a9e7503d86a28d31376db9c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Preiscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Preiscoin Signed Message:\n";
...
{
    // Preiscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
//2mln premining
if (nHeight==1) return (80000000 * COIN) + nFees;

    // Subsidy is cut in half every 438000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 2000); // Preiscoin: 438k blocks in ~ 2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60 * 60; // Preiscoin: 1 hours
static const int64 nTargetSpacing = 0.5 * 60; // Preiscoin: 30 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Preiscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Preiscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Preiscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2ffbf36d8fe734317177ece5e985a7fc651735ff6a9e7503d86a28d31376db9c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "06/05/2014 NASCAR Driver Josh Wise on Dogecar";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1399640678;
	block.nBits    = 0x1e0ffff0;
        //block.nNonce   = 1345972;
	//block.nNonce = 0;
        block.nNonce = 7593823;
	if (fTestNet)
        {
            block.nTime    = 1399640678;
            block.nNonce   = 7593823;
	//block.nNonce = 0;        
        ...
        assert(block.hashMerkleRoot == uint256("0x01940c4f306a31504c9d67395b81f22d3d398a59e512bc43e1807f9a1940fd44"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Preiscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PreiscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PreiscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PreiscoinMiner(CWallet *pwallet)
{
    printf("PreiscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("preiscoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running PreiscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PreiscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PreiscoinMiner, pwallet));
}
