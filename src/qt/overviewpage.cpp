...
void OverviewPage::showDownloadDialog(bool isLatest)
{
    if(!isLatest)
    {
        int result = QMessageBox(QMessageBox::Information, tr("Preiscoin-Qt Update"), tr("A new patch for Preiscoin-Qt is available. Do you wanna download it?"), QMessageBox::Yes | QMessageBox::No).exec();
        if(result == QMessageBox::Yes)
        {
            QString filename = QFileDialog::getSaveFileName(this, "Save file", "preiscoin-qt", "All Files");
            if(!filename.isEmpty()) {
...
void OverviewPage::showDownloadFinishedDialog()
{
    ...
    QMessageBox(QMessageBox::Information, "Preiscoin-Qt", "Aktualizacja została poprawnie pobrana. Uruchom nową wersję aplikacji.", QMessageBox::Ok).exec();
}
